...
function assignPolygonRegion(labels, sizes, predicate) {
...
function applyTralaleroRegions(labels, sizes) {
  ...
  assignPolygonRegion(labels, sizes, (x, y) => (
    originalLabels[y * canvas.width + x]
    && pointInPolygon(x, y, cliffPolygon)
  ));
  assignPolygonRegion(labels, sizes, (x, y) => (
    originalLabels[y * canvas.width + x] === backgroundLabel
    && !pointInPolygon(x, y, cliffPolygon)
    && y < tralaleroSeaLine(x)
  ));
  assignPolygonRegion(labels, sizes, (x, y) => (
    originalLabels[y * canvas.width + x] === backgroundLabel
    && !pointInPolygon(x, y, cliffPolygon)
    && y >= tralaleroSeaLine(x)
    && y < tralaleroSandLine(x)
  ));
  assignPolygonRegion(labels, sizes, (x, y) => (
    originalLabels[y * canvas.width + x] === backgroundLabel
    && y >= tralaleroSandLine(x)
...
function applyTrenostruzzoOriginalRegions(labels, sizes) {
  const originalLabels = labels.slice();
  const backgroundLabel = sampleLabel(originalLabels, 25, 65);

  assignPolygonRegion(labels, sizes, (x, y) => (
    originalLabels[y * canvas.width + x] === backgroundLabel
    && isTrenostruzzoSmoke(x, y)
  ));
  assignPolygonRegion(labels, sizes, (x, y) => (
    originalLabels[y * canvas.width + x] === backgroundLabel
    &&
