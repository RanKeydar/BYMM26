...
function findNearestRegion(startX, startY, radius) {
  ...
  for (let distance = 1; distance <= radius; distance += 1) {
    ...
    for (let x = minX; x <= maxX; x += 1) {
      ...
      if (topRegion && regionSizes[topRegion] >= 12) return { regionId: topRegion, snapped: true };
      const bottomRegion = regionMap[maxY * width + x];
      if (bottomRegion && regionSizes[bottomRegion] >= 12) return { regionId: bottomRegion, snapped: true };
    }
    for (let y = minY + 1; y < maxY; y += 1) {
      const leftRegion = regionMap[y * width + minX];
      if (leftRegion && regionSizes[leftRegion] >= 12) return { regionId: leftRegion, snapped: true };
      const rightRegion = regionMap[y * width + maxX];
      if (rightRegion && regionSizes[rightRegion] >= 12) return { regionId: rightRegion, snapped: true };
    }
  }
  return null;
}
...
function applyRegionColor(regionId, color, recordHistory = true) {
  ...
  if (virtualBarrierMask) {
    const width = canvas.width;
    for (let index = 0; index < virtualBarrierMask.length; index += 1) {
      if (!virtualBarrierMask[index]) continue;
      const x = index % width;
      const touchesRegion = (
        (index >= width && regionMap[index - width] === regionId)
        || (index + width < regionMap.length && regionMap[index + width] === regionId)
        || (x > 0 && regionMap[index - 1] === regionId)
        || (x < width - 1 && regionMap[index + 1] === regionId)
      );
